 ...
 */
public class Dactl.ArduinoUDP.Factory : GLib.Object, Dactl.Factory {
    ...
    public Dactl.Object make_object_from_node (Xml.Node *node)
        ...
        if (node->type == Xml.ElementType.ELEMENT_NODE &&
            ...
            if (node->name == "object") {
                ...
                switch (type) {
                    case "plugin-control":
                        return make_arduino_udp_plugin_control (node);
                    default:
                        throw new Dactl.FactoryError.TYPE_NOT_FOUND (
                            _("The type requested is not a known Dactl type"));
                }
    ...
    private Dactl.Object make_arduino_udp_plugin_control (Xml.Node *node) {
        return new Dactl.ArduinoUDP.Control.from_xml_node (node);
    }
}
